...
const FlashcardDashboard = () => {
  ...
  // Render functions for drag overlay (non-interactive versions)
  const renderDragOverlay = () => {
    ...
    const isGroupDrag = draggedGroupIdsRef.current !== null && draggedGroupIdsRef.current.length > 1;
    const groupCount = isGroupDrag ? draggedGroupIdsRef.current.length : 1;

    if (activeItem.type === 'folder') {
      // Count items in this folder (flat structure)
      const itemCount = items.filter(i => i.parentId === activeItem.id).length;
      return (
        <div style={{
          ...
          <div style={{
            ...
            <Folder size={48} style={{ stroke: '#ffffff', fill: 'none', strokeWidth: '2' }} />
          </div>
    ...
    } else if (activeItem.type === 'deck') {
      const deck = decks.find(d => d.id === activeItem.id);
      if (!deck) return null;
        ...
        <div style={{
          ...
          {isGroupDrag && (
            <div style={{
              ...
              fontSize: '12px',
              fontWeight: '700',
              boxShadow: '0 4px 12px rgba(0, 255, 148, 0.6)',
              zIndex: 10,
            }}>
              {groupCount}
            </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              {cardsDue > 0 ? (
                <>
                  <span style={{ fontWeight: '600' }}>{cardsDue}</span>
                  {' Cards Due'}
                </>
              ) : 'No cards due'}
            </div>
