...
const FOCUS_SESSIONS = [
  { id: 'f1', user_id: USER_ID, topic_id: 't1', title: 'Glycolysis', mode: 'pomodoro', duration_seconds: 1500, started_at: iso(0) },
...
await page.screenshot({ path: base + '-1240-collapsed.png' });

// Select Spaced Review via its tab role.
...
console.log('after click:', await page.evaluate(() => {
  ...
  return 'tabs=' + t.length + ' sel3=' + t[3]?.getAttribute('aria-selected') + ' panel=' + !!p + (p ? ' panelText=' + p.textContent.slice(0, 40) : '');
}));
await page.screenshot({ path: base + '-1240-selected.png' });

// Reduced motion: emulate, reload, capture. Nothing may move.
await page.emulateMediaFeatures([{ name: 'prefers-reduced-motion', value: 'reduce' }]);
await page.reload({ waitUntil: 'domcontentloaded' });
await new Promise((r) => setTimeout(r, 4000));
await page.screenshot({ path: base + '-1240-reduced.png' });
await page.emulateMediaFeatures([]);

// 390px vertical form + horizontal-scroll check.
await page.setViewport({ width: 390, height: 900 });
await page.reload({ waitUntil: 'domcontentloaded' });
await new Promise((r) => setTimeout(r, 4000));
console.log('390 scrollWidth vs innerWidth:', await page.evaluate(() => document.documentElement.scrollWidth + ' vs ' + window.innerWidth));
await page.screenshot({ path: base + '-390.png', fullPage: false });

// Reference renders.
const ref = await browser.newPage();
await ref.setViewport({ width: 1240, height: 700 });
await ref.goto('file:///C:/Users/johnh/MindFlow/.claude/skills/mindflow-design/reference/loop.html', { waitUntil: 'domcontentloaded' });
await new Promise((r) => setTimeout(r, 400));
await ref.screenshot({ path: base + '-ref-1240.png' });
const rtabs = await ref.$('[role="tab"]');
if (rtabs[3]) await rtabs[3].click();
await new Promise((r) => setTimeout(r, 600));
await ref.screenshot({ path: base + '-ref-1240-selected.png' });
await ref.setViewport({ width: 390, height: 900 });
await new Promise((r) => setTimeout(r, 400));
await ref.screenshot({ path: base + '-ref-390.png' });
console.log('loop captures done');
await browser.close();
